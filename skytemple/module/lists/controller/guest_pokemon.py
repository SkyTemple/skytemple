...
class GuestPokemonController(ListBaseController):
    ...
    def on_btn_help_extra_dungeon_data_clicked(self, *args):
        self._help(
            _(
                "Guest pokémon IDs refer to the entries in the guest pokémon tab. A value of -1 represents "
    ...
    def on_guest_pokemon_add_clicked(self, *args):
        ...
        store.append(
            [
                ...
                "0",
                self._get_monster_display_name(0),
                "0",
                self._get_move_display_name(0),
                self._get_move_display_name(0),
                self._get_move_display_name(0),
                self._get_move_display_name(0),
                "1",
    ...
    def _fill_extra_dungeon_data(self):
        ...
        for idx, item in enumerate(
            ...
            store.append(
                [
                    str(idx) + ": " + name,
                    str(item.guest1_index),
                    str(item.guest2_index),
                    item.hlr_uncleared,
                    item.disable_recruit,
                    item.side01_check,
                    item.hlr_cleared,
                ]
            )
        ...
        for i, item in enumerate(GuestPokemonList.read(self.arm9, self.static_data)):
            store.append(
                [
                    str(i),
                    str(item.unk1),
                    self._get_monster_display_name(item.poke_id),
                    str(item.joined_at),
                    self._get_move_display_name(item.moves[0]),
                    self._get_move_display_name(item.moves[1]),
                    self._get_move_display_name(item.moves[2]),
                    self._get_move_display_name(item.moves[3]),
                    str(item.hp),
                    str(item.level),
                    str(item.iq),
                    str(item.atk),
