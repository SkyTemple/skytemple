...
class IqController(AbstractController):
    ...
    def on_cr_other_iq_gain_edited(self, widget, path, text):
        ...
        if typ == IqGainOtherItem.NECTAR:
            self.module.project.modify_binary(
                ...
                lambda bin: HardcodedIq.set_nectar_gain(val, bin, static_data),
            )
            self.module.mark_iq_as_modified()
        elif typ == IqGainOtherItem.WONDER_GUMMI:
            self.module.project.modify_binary(
                BinaryName.ARM9,
                lambda bin: HardcodedIq.set_wonder_gummi_gain(val, bin, static_data),
            )
            self.module.mark_iq_as_modified()
    ...
    def on_cr_iq_pnts_edited(self, widget, path, text):
        store = assert_not_none(
            cast(
                ...
                builder_get_assert(
                    self.builder, Gtk.TreeView, "tree_iq_skills"
                ).get_model(),
            )
        )
        try:
        ...
        self.module.project.modify_binary(
            BinaryName.ARM9,
            lambda bin: HardcodedIq.set_iq_skills(iq_skills, bin, static_data),
        )
    ...
    def on_cr_iq_gain_edited(self, widget, path, text, *, type_id):
        store = assert_not_none(
            cast(
                Optional[Gtk.ListStore],
                builder_get_assert(
                    self.builder, Gtk.TreeView, "tree_iq_gain"
                ).get_model(),
            )
        )
        try:
    ...
    def on_cr_belly_heal_edited(self, widget, path, text, *, type_id):
        store = assert_not_none(
            cast(
                Optional[Gtk.ListStore],
                builder_get_assert(
                    self.builder, Gtk.TreeView, "tree_belly_gain"
                ).get_model(),
            )
        )
        try:
    ...
    def on_cr_skill_to_group(self, widget, path, *, group_id):
        ...
        self.module.project.modify_binary(
            BinaryName.ARM9,
            lambda bin: IqGroupsSkills.write_compressed(bin, groups, static_data),
        )
    ...
    def _init_misc_settings(self):
        ...
        builder_get_assert(
            self.builder, Gtk.Entry, "entry_min_iq_exclusive_move_user"
        ).set_text(
            str(HardcodedIq.get_min_iq_for_exclusive_move_user(arm9, static_data))
        )
        builder_get_assert(
            self.builder, Gtk.Entry, "entry_min_iq_item_master"
        ).set_text(str(HardcodedIq.get_min_iq_for_item_master(arm9, static_data)))
        builder_get_assert(
            self.builder, Gtk.Entry, "intimidator_activation_chance"
        ).set_text(str(HardcodedIq.get_intimidator_chance(ov10, static_data)))
    ...
    def _init_iq_gains(self):
        ...
        # Other items
        store_other_items = builder_get_assert(
            self.builder, Gtk.ListStore, "iq_gain_other_items"
        )
