...
class WorldMapController(AbstractController):
    ...
    def get_view(self) -> Gtk.Widget:
        ...
        self._change_map_bg(
            self._level_id,
            builder_get_assert(self.builder, Gtk.DrawingArea, "draw"),
            self.drawer,
        )
    ...
    def on_draw_event_button_press_event(self, box, button: Gdk.EventButton):
        ...
        if button.button == 1:
            ...
            if selected is not None:
                tree: Gtk.TreeView
                tree, l_iter = (
                    builder_get_assert(self.builder, Gtk.TreeView, "tree"),
    ...
    def _edit(self):
        ...
        if model is not None and treeiter is not None:
            ...
            # Map combobox
            cb_map: Gtk.ComboBox = builder_get_assert(
                self.builder, Gtk.ComboBox, "cb_map"
            )
            ...
            # Reference combobox
            cb_reference: Gtk.ComboBox = builder_get_assert(
                self.builder, Gtk.ComboBox, "cb_reference"
            )
            ...
            if self._markers[idx].reference_id > -1:
                builder_get_assert(
                    self.builder, Gtk.RadioButton, "radio_reference"
                ).set_active(True)
            else:
                builder_get_assert(
                    self.builder, Gtk.RadioButton, "radio_pos"
                ).set_active(True)

            # Drawer
            self._change_map_bg(
                self._markers[idx].level_id,
                builder_get_assert(self.builder, Gtk.DrawingArea, "diag_draw"),
                self.dialog_drawer,
            )
            ...
            self.on_radio_reference_toggled(
                builder_get_assert(self.builder, Gtk.RadioButton, "radio_reference")
            )
            ...
            if response == Gtk.ResponseType.APPLY:
                ...
                if model is not None and cbiter is not None and cbiter != []:
                    ...
                    return
                use_reference = builder_get_assert(
                    self.builder, Gtk.RadioButton, "radio_reference"
                ).get_active()
                reference_id_selected = -1
                if use_reference:
                    cb_reference = builder_get_assert(
                        self.builder, Gtk.ComboBox, "cb_reference"
                    )
                    model, cbiter = (
