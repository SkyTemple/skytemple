        ...
        for idx, entry in enumerate(self._list):
            self._list_store.append(
                [
                    ...
                    str(entry.level),
                    str(entry.joined_at),
                    self._get_icon(entry.poke_id, idx, False),
                    entry.poke_id,
                    self._location_names[entry.joined_at],
                    self._ent_names[entry.poke_id],
                    self._get_move_display_name(entry.move1),
                    self._get_move_display_name(entry.move2),
                    self._get_move_display_name(entry.move3),
                    self._get_move_display_name(entry.move4),
                    bool(entry.do_not_fix_entire_moveset),
                    str(entry.iq),
                    str(entry.fixed_hp),
                ]
            )
