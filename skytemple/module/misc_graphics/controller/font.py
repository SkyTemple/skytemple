...
class FontController(AbstractController):
    ...
    def on_btn_add_clicked(self, widget):
        ...
        builder_get_assert(self.builder, Gtk.SpinButton, "entry_char_id").set_text(
            str(0)
        )
        builder_get_assert(
            self.builder, Gtk.SpinButton, "entry_char_id"
        ).set_increments(1, 1)
        builder_get_assert(self.builder, Gtk.SpinButton, "entry_char_id").set_range(
            0, 255
        )
        ...
        if resp == ResponseType.OK:
            ...
            try:
                ...
                entry_tree = builder_get_assert(
                    self.builder, Gtk.TreeView, "entry_tree"
                )
                store = assert_not_none(
                    cast(Optional[Gtk.ListStore], entry_tree.get_model())
                )
