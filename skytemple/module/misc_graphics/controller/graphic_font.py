...
class GraphicFontController(AbstractController):
    ...
    def on_import_clicked(self, w: Gtk.MenuToolButton):
        ...
        if response == Gtk.ResponseType.ACCEPT:
            ...
            builder_get_assert(
                self.builder, Gtk.SpinButton, "nb_entries_import"
            ).set_increments(1, 1)
            builder_get_assert(
                self.builder, Gtk.SpinButton, "nb_entries_import"
            ).set_range(1, MAX_ENTRIES - 1)
            builder_get_assert(
                self.builder, Gtk.SpinButton, "nb_entries_import"
            ).set_text(str(self.font.get_nb_entries()))
    ...
    def _switch_entry(self):
        surface = self.font.get_entry(
            int(builder_get_assert(self.builder, Gtk.SpinButton, "entry_id").get_text())
        )
