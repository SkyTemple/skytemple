...
def on_map_width_chunks_changed(builder, wdg):
    ...
    if map_wh_link.get_active():
        map_width_tiles: Gtk.Entry = builder.get_object("map_width_tiles")
        try:
...
def on_map_height_chunks_changed(builder, wdg):
    ...
    map_wh_link: Gtk.Switch = builder.get_object("map_wh_link")
    if map_wh_link.get_active():
        map_height_tiles: Gtk.Entry = builder.get_object("map_height_tiles")
        try:
...
def on_map_wh_link_state_set(builder, wdg, state):
    ...
    map_wh_link_target: Gtk.Switch = builder.get_object("map_wh_link_target")
    map_wh_link_target.set_sensitive(not state)
    if state:
        on_map_width_chunks_changed(builder, builder.get_object("map_width_chunks"))
        on_map_height_chunks_changed(builder, builder.get_object("map_height_chunks"))
