...
from typing import TYPE_CHECKING, List, Type, Dict, Tuple, Optional, cast
...
class StDungeonFloorPage(Gtk.Box):
    ...
    def on_btn_help_initial_enemy_density_clicked(self, *args):
        self._help(
            _(
                "The final value is randomized between randomized density / 2 and density - 1 if the value is positive. If it's negative, its absolute value is used (no random variation is applied in this case)."
            )
        )
    ...
    def on_btn_help_max_coin_amount_clicked(self, *args):
        self._help(
            _(
                "The value stored in the ROM is actually the value divided by 5. Because of this the saved value will be rounded to the next multiple of 5."
            )
        )
