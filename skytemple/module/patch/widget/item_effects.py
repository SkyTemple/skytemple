...
@LocalePatchedGtkTemplate(filename=os.path.join(data_dir(), "widget", "patch", "item_effects.ui"))
class StPatchItemEffectsPage(Gtk.Stack):
    __gtype_name__ = "StPatchItemEffectsPage"
    module: PatchModule
    item_data: None
    effect_ids_store: Gtk.ListStore = cast(Gtk.ListStore, Gtk.Template.Child())
    effect_items_store: Gtk.ListStore = cast(Gtk.ListStore, Gtk.Template.Child())
    item_effects_store: Gtk.ListStore = cast(Gtk.ListStore, Gtk.Template.Child())
    ...
    def _init_item_list(self):
        ...
        item_store = self.item_effects_store
        # Init list
        item_store.clear()
        non_sorted = []
    ...
    def on_btn_add_effect_clicked(self, *args):
        self.item_effects.add_effect_code(bytes([100, 9, 0, 234]))  # Branch to the end
